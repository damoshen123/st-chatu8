...
由 Codex 协助 yunx738 修改，2026-09-13；本节及相关修复沿用 [Aladdin Free Public License](LICENSE)。

ACU 更新表格时可能删除并重建条目，使数字 UID 分配给其他条目。本扩展对 ACU 生成的条目按同一世界书内的完整名称保存普通启用、强制启用、禁用和角色绑定，并保留 `ACU-[聊天标识]-` 前缀，以便名称不变时在重建后继续选择原条目。发送数据和资料库共用这套规则，普通世界书条目继续按 UID 保存。

首次升级后，打开「发送数据」→「刷新世界书」，核对需要的 DNA 条目及其强制启用状态，再点击「测试触发」。旧 UID 配置在首次读取对应世界书时迁移；如果升级前已经串位，需要手动纠正一次。更新其他表格后可再次刷新、测试，检查原选择是否保留。

此规则依赖条目名称唯一且稳定。条目改名、同一世界书内重名，或「表名-行号」因行移动而指向其他人物时，仍需核对并重新选择；同名条目以 UID 区分。本修复不改变世界书内容、表顺序、`depth`、`order` 或注入位置。

`world-entry-selection.mjs` 是运行时依赖，须与 `index.js` 保持在同一目录。回归测试位于 `tests/`，无需额外 npm 依赖，在仓库根目录运行：

```sh
node --test tests/world-entry-selection.test.mjs
```

现有 10 项测试覆盖 UID 复用、重建空窗、配置重新加载、聊天隔离、角色绑定及触发规则；它们在 Node 环境中执行实际业务函数，完整 SillyTavern 界面的刷新与发送仍需手动验证。
